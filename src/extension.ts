...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-phar.helloWorld2', () => {
			...
			vscode.workspace.updateWorkspaceFolders(0, 0, { uri: vscode.Uri.parse('phar:///C:/local_disk/zobo/Projects/vscode-php-debug/vscode-phpactor/phpactor.phar'), name: "phpactor.phar" });
		})
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vscode-phar.open-phar', async (uri: vscode.Uri | undefined) => {
			...
			if (internalPath == '') {
				vscode.workspace.updateWorkspaceFolders(0, 0, { uri: uri.with({scheme: 'phar'}), name: path.basename(uri.path) });
			} else {
				let doc = await vscode.workspace.openTextDocument(uri); // calls back into the provider
				await vscode.window.showTextDocument(doc, { preview: false });
			}
	...
	context.subscriptions.push(
		vscode.workspace.onDidOpenTextDocument((document) => {
			if (document.languageId == 'phar') {
				...
				vscode.workspace.updateWorkspaceFolders(0, 0, { uri: document.uri.with({scheme: 'phar'}), name: path.basename(document.uri.path) });
			}
		})
	)
}
...
class PharFsProvider implements vscode.FileSystemProvider
{
	...
	async readDirectory(uri: vscode.Uri): Promise<[string, vscode.FileType][]> {
		...
		const d = fe.map<[string, vscode.FileType]>((f) => [f.name, f.directory ? vscode.FileType.Directory : vscode.FileType.File])
